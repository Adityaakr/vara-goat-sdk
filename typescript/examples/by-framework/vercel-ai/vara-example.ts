...
async function main() {
    try {
        ...
        const mnemonic = process.env.VARA_MNEMONIC || "test";
        ...
        async function handleVaraTransfer(to: string, amount: string) {
            try {
                ...
                const tx = wallet.api.tx.balances.transferKeepAlive(to, amountInSmallestUnit);
            ...
            try {
                const result = await generateText({
                    ...
                    tools: {
                        sendVara: tool({
                            description:
                                "Send VARA tokens to another address. The amount should be in whole VARA tokens (e.g., '1' means 1 VARA token, '0.5' means half a VARA token)",
                            parameters: z.object({
                                to: z.string().describe("Recipient address"),
                                amount: z
                                    .string()
                                    .describe("Amount of VARA tokens to send (e.g., '1' for 1 VARA token)"),
                            }),
                        ...
                        checkBalance: tool({
                            description:
                                "Check the VARA token balance of a wallet address. If no address is provided, it checks the balance of the current wallet.",
                            parameters: z.object({
                                address: z
                                    .string()
                                    .optional()
                                    .describe(
                                        "Optional wallet address to check. If not provided, checks the current wallet's balance.",
                                    ),
                            }),
