...
async function main() {
    try {
        const mnemonic = process.env.VARA_MNEMONIC || "test test test test test test test test test test test junk";
        ...
        async function handleVaraTransfer(to: string, amount: string) {
            try {
                ...
                const tx = wallet.getApi().tx.balances.transferKeepAlive(
                    to,
                    amountInSmallestUnit
                );
                const hash = await new Promise<string>((resolve, reject) => {
                    tx.signAndSend(wallet["kp"], (result: any) => {
                        if (result.status.isFinalized) resolve(result.txHash.toHex());
                        if (result.isError) reject(result);
                    });
                });
                return {
                    success: true,
        ...
        async function checkWalletBalance(address?: string) {
            try {
                ...
                const { data: balance } = await wallet.getApi().query.system.account(targetAddress);
        ...
        // Disconnect wallet
        await wallet.getApi().disconnect();
    } catch (error) {
